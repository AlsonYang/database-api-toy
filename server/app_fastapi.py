...
def get_member(
    id: int = Path(
        None, describe="The ID of the member you'd like to get", gt=0, lt=9999
    )
):  # The Path is used to specify description and value constraints for the path parameter.
    """."""
...
def edit_member(id: int, member_data: MemberData):
    ...
    response = DatabaseEditor.put(
        id, name=member_data.name, description=member_data.description
    )
    return standardise_db_response(response)
